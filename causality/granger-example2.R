...
# 日米 2年国債利回り
g <- ggplot(NULL)
g <- g + geom_line(data=bond.us, aes(x=stock[,1], y =two_years), colour="cyan")
g <- g + geom_line(data=bond.df, aes(x=stock[,1], y =value), colour="magenta") + labs(title="", x="", y="日米 2年国債利回り (%)")
plot(g)

#日米 2年国債利回り 差分
bond.diff <- data.frame(date=as.Date(bond.df[,1]), value=bond.us[,3]-bond.df[,2])
g <- ggplot(NULL)
g <- g + geom_line(data=bond.diff, aes(x=date, y=value)) + labs(title="", x="", y="日米 2年国債利回りの差分 (%)")
plot(g)
...
g <- ggplot(NULL)
g <- g + geom_line(data=df, aes(x=date, y=scale(interest_diff)), colour="cyan")
g <- g + geom_line(data=df, aes(x=date, y=scale(stock)), colour="magenta") + labs(title="", x="", y="日米金利差, 日経平均株価 (標準化)")
plot(g)
