...
g <- g + geom_line(colour="magenta") + labs(title="", x="", y="日本10年国債利回り (%)")
plot(g)
...
g <- g + geom_line(colour="magenta") + labs(title="", x="", y="インフレ率 (%)")
plot(g)
...
g <- ggplot(NULL)
g <- g + geom_line(data=stock, aes(x=date, y=close))
plot(g)
...
g <- ggplot(NULL)
g <- g + geom_line(data=interest.df, aes(x=date, y=value))
plot(g)
...
g <- ggplot(NULL)
g <- g + geom_line(data=df, aes(x=date, y=scale(real_interest)), colour="cyan")
g <- g + geom_line(data=df, aes(x=date, y=scale(stock)), colour="magenta") + labs(title="", x="", y="実質金利, 日経平均株価 (標準化)")
plot(g)
...
g <- ggplot(NULL)
g <- g + geom_line(data=df.part, aes(x=date, y=scale(real_interest)), colour="cyan")
g <- g + geom_line(data=df.part, aes(x=date, y=scale(stock)), colour="magenta") + labs(title="", x="", y="実質金利, 日経平均株価 (標準化)")
plot(g)
